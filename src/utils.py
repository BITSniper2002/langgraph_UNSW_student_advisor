    ...
    if isinstance(message.content, str):
        ...
        for item in message.content:
            if item.get("type") == "text":
                ...
                if item['name'] in ["read_file", "write_file"]:
                    continue  # Skip displaying file operation tool calls
                parts.append(f"\n🔧 Tool Call: {item['name']}")
...
def format_messages(messages):
    ...
    for m in messages:
        ...
        if msg_type == "Human":
            ...
            # Check if this is a file operation tool output
            if hasattr(m, 'name') and m.name in ["read_file", "write_file"]:
                continue  # Skip displaying file operation outputs
